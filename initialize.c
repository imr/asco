...
/*
 * receives a string with #<text># and replaces it with the correct parameters
...
/*
 * receives a string with #SYMBOL# and/or #NODE# text and replaces it with the correct symbol
 */
char *DecodeSymbolNode(char *ret, int i)
{
	...
	while (ii<=strlen(lkk)) {
		...
		if (!strcmp(laux, "symbol"))
			ii=ii+1000;                   //is a "symbol" and codify this information by adding '1000'
		else {
...
int initialize(char *filename) //, double *x)
{
	...
	if (strcmp(lkk, "# Parameters #")) {
		...
		while ((lkk[0] != '#') && (lkk[0] != '\0') && (ii=strpos2(lkk, ":", 1)) && (!feof(fspice_cfg))) {
			if (lkk[0] != '*') {
				...
				parameters[i].format=-1;        //'-1' is used for posterior value validation
				char laux2[SHORTSTRINGSIZE];
	...
	if (strcmp(lkk, "# Measurements #")) {
		...
		while ((lkk[0] != '#') && (lkk[0] != '\0') && (ii=strpos2(lkk, ":", 1)) && (!feof(fspice_cfg))) {
			if (lkk[0] != '*') {
				...
				measurements[i].objective_constraint=0;        //'0' is used for posterior value validation
				if (!strcmp(laux, "MIN"))
	...
	int  ccode;
	if ((ccode = gethostname(hostname, sizeof(hostname))) != 0) { //!=0 can most likelly be deleted from all if lines
		...
		if (strcmp(laux, ".end")) {
			...
			if (lkk[0]!='*') {
				...
				if (laux[0]=='\0') {                              //does it contains #<text>#?
					...
					ReplaceSymbol(lkk, 0);
					fprintf(fspice_tmp, "%s\n", lkk);         //write line to <hostname>.cir
				}
			}
			// ***** -------------- **  END  ** -------------- *****
			// ***** -------------- *********** -------------- *****
	...
	if (strcmp(laux, ".end") && (spice < 100)) { //if it is really a spice simulation and not a general file, then exit if ".end" is not found
		printf("initialize.c - Step4 -- End not found in <inputfile>.*\n");
		exit(EXIT_FAILURE);
	}

	//Step4.2: ".end" is found and now add measurements
	i=0;
	fprintf(fspice_tmp, "\n");
			...
			switch(spice) {
				case 1: //Eldo
					...
					if (strpos2(laux, ".meas ", 1)) {
						...
						sprintf(lkk, "%s%s", "extract/", measurements[i].meas_symbol); //to remove integer added in (*)
						lkk[strlen(lkk)-ii]='\0';                                      //to remove integer added in (*)
						printf("initialize.c - Step4.2.1 -- .MEAS not supported in file: %s\n", lkk);
						exit(EXIT_FAILURE);
					}
					break;
				case 2: //HSPICE
	...
	//Step4.2.2: Add entry to variable 'measure[i].var_name' and 'measure[i].search'
		...
		if (strcmp((sprintf(laux, "%.11s", lkk), laux), "MEASURE_VAR")) { //general case if no "MEASURE_VAR" exist in "/extract/<file>"
			sprintf(lkk, "%s%s", UNIQUECHAR,measurements[i].meas_symbol);
			strcpy(measure[ii].var_name, lkk); //measure[i].var_name
			...
			strcpy(measure[ii].search, lkk);   //measure[i].search
	...
	if (spice < 100) //if it is really a spice simulation and not a general file, then write the ".end" line
		fprintf(fspice_tmp, "%s\n", ".end");
